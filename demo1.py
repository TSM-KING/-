...
# 返回训练过程中的评估结果
evals_result = {}
bst = xgb.train(params, dtrain, num_boost_round=50, evals=evals, early_stopping_rounds=10, evals_result=evals_result, verbose_eval=True)
...
df = pd.DataFrame({
    ...
    'Predicted Label': [label_names[label] for label in y_pred_int]
})
...
# 创建 AdaBoost 模型，基学习器为决策树桩（树的最大深度为1）
base_estimator = DecisionTreeClassifier(max_depth=1)
adaboost_model = AdaBoostClassifier(base_estimator=base_estimator, n_estimators=20, random_state=42)

# 训练 AdaBoost 模型
adaboost_model.fit(X_train_flat, y_train)

# 在验证集上进行预测
y_pred_val = adaboost_model.predict(X_val_flat)
y_pred_test = adaboost_model.predict(X_test_flat)
...
disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=label_names)
disp.plot(cmap='Blues', values_format='d')
plt.title('Confusion Matrix for AdaBoost (CIFAR-10)')
plt.show()
...
# 标准化数据（对于决策树来说，虽然不总是必要，但可以提高模型的稳定性）
...
# 绘制混淆矩阵
cm = confusion_matrix(test_labels, y_pred_test)
disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=load_cifar10_meta(data_dir))
disp.plot(cmap='Blues', values_format='d')
plt.title('Confusion Matrix for Random Forest (CIFAR-10)')
plt.show()
